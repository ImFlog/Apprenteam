<?php require 'header.html'; ?>

<div class="container">
	<div class="row row-right main-content">
		<div class="col-xs-12 col-sm-9">

			<h1> L'apprentissage en mécanique </h1>

			<div class="col-sm-4 image"> 
				<img src="static/img/computers.jpg" class="img-responsive" alt="Apprenteam"/>
			</div>
			<p>L'apprentissage en mécanique a été créé à la rentrée 2008. Pionnière de l'apprentissage à l'UTC, elle est maintenant confortablement installée au sein de l'UTC. 
				Le diplôme obtenu en apprentissage est exactement le même qu'en cursus classique.
			</p>

		</p>

		<p>
			<h3> Deux parcours sont proposées : </h3>
			<ul>
				<li> Conception</li> 
				<li> Industrialisation </li>
			</ul>
		</p>

		<h3> Les apprentis vous expliquent qui ils sont et ce qu'ils font ! </h3>
	</br>

</div>

<?php require 'sidebar.html';?>


<div class="row">
	<iframe width="640" height="360" src="//www.youtube.com/embed/ZbpulwbeQ5I" frameborder="0" allowfullscreen></iframe>
</div>

<div class="row main-content">
	<h3> Cours dispensés </h3>
	<p> Chaque année, entre 8 et 10 enseignements sont à choisir parmis cette liste.</p>
</div>
<div class="row main-content">
	<div class="col-sm-6">


		<TABLE BORDER="1"> 
			<CAPTION> <h4> Enseignements de 1ère année <h4> </caption>
				<TR> 
					<TH> Code </TH> 
					<TH> Type </TH> 
					<TH> Dénomination </TH> 
				</TR> 
				<TR> 
					<TD> EN21 </TD> 
					<TD> CS </TD> 
					<TD> Base de l'électronique analogique </TD> 
				</TR> 

				<TR> 
					<TD> MA11 </TD> 
					<TD> CS </TD> 
					<TD>Révision d'analyse et d'algèbre linéaire</TD> 
				</TR> 

				<TR> 
					<TD> MT09 </TD> 
					<TD> CS </TD> 
					<TD> Analyse numérique </TD> 
				</TR> 

				<TR> 
					<TD> MT12 </TD> 
					<TD> CS </TD> 
					<TD> Méthodes mathématiques pour l'analyse du signal et de l'image </TD> 
				</TR> 

				<TR> 
					<TD> MQ01 </TD> 
					<TD> CS </TD> 
					<TD> Element de resistance des matériaux </TD> 
				</TR> 

				<TR> 
					<TD> MQ03 </TD> 
					<TD> CS </TD> 
					<TD> Mécanique des vibrations 1</TD> 
				</TR> 

				<TR> 
					<TD> MQ17 </TD> 
					<TD> CS </TD> 
					<TD> Introduction aux propriétés mécaniques et à l'ingéniereie des matériaux </TD> 
				</TR> 

				<TR> 
					<TD> FQ01 </TD> 
					<TD> TM </TD> 
					<TD> Economie globale et maitrise de la qualité</TD> 
				</TR> 

				<TR> 
					<TD> GE37 </TD> 
					<TD> TM </TD> 
					<TD>Gestion de projet </TD> 
				</TR> 

				<TR> 
					<TD>DI05</TD> 
					<TD> TM </TD> 
					<TD> Méthodologie et analyse de la valeur </TD> 
				</TR> 

				<TR> 
					<TD> NF02 </TD> 
					<TD> TM </TD> 
					<TD> Du circuit integré au mucroprocesseur </TD> 
				</TR> 

				<TR> 
					<TD> LO01 </TD> 
					<TD> TM </TD> 
					<TD> Bases de la programmation </TD> 
				</TR> 

				<TR> 
					<TD> NF22 </TD> 
					<TD> TM </TD> 
					<TD> Micro-ordinateurs et applications</TD> 
				</TR> 

				<TR> 
					<TD> TN02 </TD> 
					<TD> TM </TD> 
					<TD> Introduction à la conception mécanique</TD> 
				</TR> 

				<TR> 
					<TD> TN12 </TD> 
					<TD> TM </TD> 
					<TD>Conception mécanique </TD> 
				</TR> 

				<TR> 
					<TD> TN22 </TD> 
					<TD> TM </TD> 
					<TD> Eléments de bureau d'études</TD> 
				</TR> 

				<TR> 
					<TD> MQ02 </TD> 
					<TD> CS </TD> 
					<TD> Mécanique des solides déformables </TD> 
				</TR> 

				<TR> 
					<TD> SY04 </TD> 
					<TD> CS </TD> 
					<TD> Systèmes asservis linéaires : analyse et commande</TD> 
				</TR> 


			</TABLE> 
		</div>
		<div class="col-sm-6">
			<TABLE>

				<CAPTION> <h4> Enseignements de 2ème année <h4> </caption>
					<TR> 
						<TH> Code </TH> 
						<TH> Type </TH> 
						<TH> Dénomination </TH> 
					</TR> 
					<TR> 
						<TD> API01 </TD> 
						<TD> CS </TD> 
						<TD> Java EE et architecture WEB </TD> 
					</TR> 

					<TR> 
						<TD> API02  </TD> 
						<TD> CS </TD> 
						<TD> Architecture des réseaux</TD> 
					</TR>

					<TR> 
						<TD> API03 </TD> 
						<TD> TM </TD> 
						<TD> Capteurs intelligents communicants</TD> 
					</TR>

					<TR> 
						<TD> API04  </TD> 
						<TD> TM </TD> 
						<TD>  Data warehouse et outils decisionnels </TD> 
					</TR>

					<TR> 
						<TD> API05 </TD> 
						<TD> CS </TD> 
						<TD> Modelisation et simulation de flux </TD> 
					</TR>

					<TR> 
						<TD> API06  </TD> 
						<TD> CS </TD> 
						<TD> Analyse des données et data mining </TD> 
					</TR>

					<TR> 
						<TD> API07 </TD> 
						<TD> TM </TD> 
						<TD> Architecture des applications internet, programmation web et sécurité</TD> 
					</TR>

					<TR> 
						<TD> API08 </TD> 
						<TD> TM </TD> 
						<TD> IHM et multimédia</TD> 
					</TR>


					<TR> 
						<TD> API09 </TD> 
						<TD> CS </TD> 
						<TD> Méthode et outils pour l'optimisation et la simulation </TD> 
					</TR>


					<TR> 
						<TD> API10 </TD> 
						<TD> TM </TD> 
						<TD> Méthodes de vérification et validation de logiciel et progrmamation sous linux</TD> 
					</TR>


				</TABLE>
			</br> </br> </br> </br>
			<TABLE>
				<CAPTION> <h4> Enseignements de 3ème année <h4> </caption>
					<TR> 
						<TH> Code </TH> 
						<TH> Type </TH> 
						<TH> Dénomination </TH> 
					</TR> 
					<TR> 
						<TD> API11 </TD> 
						<TD> CS </TD> 
						<TD> Java EE et architecture WEB </TD> 
					</TR> 

					<TR> 
						<TD> API12 </TD> 
						<TD> CS </TD> 
						<TD> Java EE et architecture WEB </TD> 
					</TR> 

					<TR> 
						<TD> API13 </TD> 
						<TD> CS </TD> 
						<TD> Java EE et architecture WEB </TD> 
					</TR> 

					<TR> 
						<TD> API14 </TD> 
						<TD> CS </TD> 
						<TD> Java EE et architecture WEB </TD> 
					</TR> 

					<TR> 
						<TD> API15 </TD> 
						<TD> CS </TD> 
						<TD> Java EE et architecture WEB </TD> 
					</TABLE>
				</TR> 
			</div>
		</div>

	</div>
	<?php require 'footer.html'; ?>
